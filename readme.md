...
# Argos+Gramine Quick Build

Below are instructions for quickly building Argos+Gramine along with its example FHE applications.
...
Argos+Gramine does not contain the same changes to Tyche that Argos (on the `main` branch) does.
Specifically, it does not have support for interacting with the TPM, and there are no vmcalls for creating a transcript.
It is mostly useful for making development of enclave applications faster, and for benchmarking our custom runtime against.

## Building example SEAL FHE applications

Instructions for building and running our example SEAL FHE applications under Argos+Gramine can be found in the [`argos-gramine-testbench`][https://github.com/mit-enclaves/argos-gramine-testbench] repository.
