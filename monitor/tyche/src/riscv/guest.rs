...
use riscv_sbi::sbi::{EXT_RFENCE, EXT_TIME};
use super::monitor;
...
pub fn handle_exit(reg_state: &mut RegisterState) {
    ...
    log::debug!(
        ...
        mcause::ECALL_FROM_SMODE => {
            if reg_state.a7 == 0x5479636865 {
                ...
                //if(reg_state.a7 == EXT_RFENCE) {
                //    log::info!("Return values from ecall: ret: {:x} out_val: {:x} a6: {:x}", ret, out_val, reg_state.a6);  
                //}
            }
        }
    ...
    // TODO: This shouldn't happen in case of an interrupt either. 
    if (mcause & (1 << 63)) != (1 << 63) {
    unsafe {
        asm!("csrr t0, mepc");
        asm!("addi t0, t0, 0x4");
