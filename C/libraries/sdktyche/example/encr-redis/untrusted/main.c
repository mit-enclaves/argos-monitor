...
void* run_domain(void* arg) {
  ...
  if (pin_self_to_core(args->core) != SUCCESS) {
      ERROR("Pinning in debug thread %lld did not work.", args->core);
      goto failure;
  }
  // We only need one thread for the debugging.
  if (args->debug_role == REDIS_DOMAIN) {
    LOG("Debug redis thread exiting as planned");
    return NULL;
  }
  // main loop, read on a channel, write on the other.
  while(1) {
    // Start by reading.
    while((read = rb_char_read_n(&(args->debug_chan->request),
            MSG_BUFFER_SIZE, buffer)) == 0) {
      //TODO: if we want to sleep, do it here.
    }
    if (read == FAILURE) {
      ERROR("Reading in debug thread returned a failure.");
      goto failure;
    }
    printf("debug_in: %s\n", buffer);
    // Now write.
    written = 0;
    while(written < read) {
      int res = rb_char_write_n(&(args->debug_chan->response), read - written, &buffer[written]);
      if (res == FAILURE) {
        ERROR("Failed to write to the response channel");
        goto failure;
      }
      written += res;
    }
...
static int core_allocation(usize *untrusted_core, usize domains_core[NB_DOMAINS]) {
  ...
  // the SDK-Tyche requires 3 cores: 1) untrusted, 2) encr, 3) redis.
...
int main(int argc, char *argv[]) {
  ...
  char* domains_paths[NB_DOMAINS] = {"encr_enclave", "redis_enclave"};
  ...
  encr_comm = (encr_channel_t*) find_default_shared(domains[ENCR_DOMAIN]);
  if (encr_comm == NULL) {
    ERROR("Unable to find the default shared region in dom %d.", ENCR_DOMAIN);
    goto failure;
  }
  // Initialize the channels.
  if (rb_char_init(&(encr_comm->request), MSG_BUFFER_SIZE,
        encr_comm->request_buffer) != SUCCESS) {
    ERROR("Problem in the init of the request channel.");
    goto failure;
  }
  if (rb_char_init(&(encr_comm->response), MSG_BUFFER_SIZE,
        encr_comm->response_buffer) != SUCCESS) {
    ERROR("Problem in the init of response channel.");
    goto failure;
  }
  memset(encr_comm->request_buffer, 0, sizeof(char) * MSG_BUFFER_SIZE);
  memset(encr_comm->response_buffer, 0, sizeof(char) * MSG_BUFFER_SIZE);
  
  // Run the domain threads.
  // TODO: we need some kind of synchronization to init the channel
  // between encr and redis OR we make sure the pipe is mmaped to 0.
  for (int i = 0; i < NB_DOMAINS; i++) {
...
#ifdef RUN_TCP
  // run the tcp server.
  if (tcp_start_server(untrusted_core, encr_comm) != SUCCESS) {
    ERROR("TCP server failed.");
    goto failure;
  }
#else
  // By default we run the stdin version.
  if (stdin_start_server(untrusted_core, encr_comm) != SUCCESS) {
