...
// The virtual address at which the ssl has its shared state with tychools.
#define SSL_VIRT_ADDRESS 0x301000ULL

/// A two way channel used:
/// 1) By the untrusted code to forward/receive encrypted request/response.
/// 2) By the ssl and redis enclaves to forward/receive requests/response.
...
typedef two_way_channel_t ssl_channel_t;
