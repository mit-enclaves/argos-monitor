...
use riscv_utils::{read_mscratch, RegisterState, HART_START, HART_START_ADDR, HART_START_ARG1, NUM_HARTS, AVAILABLE_HART_MASK, HART_IPI_SYNC, IPI_TYPE_TLB, IPI_TYPE_SMODE}; 
use capa_engine::Buffer;
use crate::{TYCHE_SBI_VERSION, ECALL_IMPID, ECALL_VERSION_MAJOR, ECALL_VERSION_MINOR, SPEC_VERSION_MAJOR_MASK, SPEC_VERSION_MAJOR_OFFSET};

use crate::ipi::{aclint_mswi_send_ipi, process_ipi, ipi_handling_failed, process_tlb_ipi};
use crate::{sbi, sbi_ext_base, sbi_ext_hsm, sbi_ext_ipi, sbi_ext_rfence, IPIRequest};
    ...
    if a6 == sbi_ext_ipi::SEND_IPI {
        ...
        if a1 == -1 {
            ...
            for i in a1.try_into().unwrap()..NUM_HARTS {
                if (((available_hart_mask & 0x1) & (target_hart_mask & 1)) == 1) && (i != src_hartid) {
                    
                    //TODO: WHAT ABOUT CURRENT HART? No such check in OpenSBI - let's see if it's
                    //needed.   
                    //log::info!("Hmask harts: Sending IPI to hart {} a0: {}, a1: {}", i, a0, a1);
                    //let mut ipi_requests = HART_IPI_BUFFER[i as usize].lock();
                    //ipi_requests.push(IPIRequest::SMode);
                    //drop(ipi_requests); 
                    IPI_TYPE_SMODE[i].store(true, Ordering::SeqCst); 
...
pub fn sbi_ext_rfence_handler(ret: &mut isize, _err: &mut usize, out_val: &mut usize, a0: usize, a1: isize, start: usize, size: usize, asid: usize, a6: usize) {
    ...
    match a6 {
                ...
                for i in a1.try_into().unwrap()..NUM_HARTS {
                    if ((available_hart_mask & 0x1) & (target_hart_mask & 1)) == 1 {
                        if i == src_hartid {
                            ...
                            //Send IPI to the hart.
                            HART_IPI_SYNC[src_hartid].fetch_add(1, Ordering::SeqCst); 
                            //log::info!("Sending IPI to hart {} from hart {}", src_hartid, i);
                            IPI_TYPE_TLB[i].store(true, Ordering::SeqCst); 
                            aclint_mswi_send_ipi(i);
                            //HART_IPI_SYNC[src_hartid].fetch_add(1, Ordering::SeqCst); 
                        }
        ...
        //sbi_ext_rfence::REMOTE_FENCE_I => ecall_handler_failed(),
    ...
    while HART_IPI_SYNC[src_hartid].load(Ordering::SeqCst) > 0 {
        ...
        if let Some(ipi_req) = ipi_requests.pop() {
            match ipi_req {
                IPIRequest::RfenceSfenceVMAASID { src_hartid, start, size, asid } => {
