...
impl Context {
    ...
    pub fn root(&mut self, start: usize, end: usize) -> Handle<Region> {
        // TODO: check validity
        let region = Region {
            start,
            end,
    ...
    pub fn alias(&mut self, parent: Handle<Region>, start: usize, end: usize) -> Handle<Region> {
        // TODO: check validity
        let region = Region {
            start,
            end,
    ...
    pub fn carve(&mut self, parent: Handle<Region>, start: usize, end: usize) -> Handle<Region> {
        // TODO: check validity
        let region = Region {
            start,
            end,
...
impl fmt::Debug for Context {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        for region in &self.regions.store {
            match region.kind {
                RegionKind::Root => {
                    writeln!(f, "  r{} = root {} {}", idx, region.start, region.end)?
                }
