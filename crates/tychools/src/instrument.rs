...
pub fn instrument_binary(manifest: &Manifest, riscv_enabled: bool, vf2_enabled: bool) {
    ...
    let main_conf = if let (Some(ref mut user), Some(kern)) = (&mut user_elf, &mut kern_elf) {
        ...
        if manifest.generate_pts {
            user.generate_page_tables(manifest.security, &manifest.map_page_tables, riscv_enabled, vf2_enabled);
        }
        user
        ...
        if manifest.generate_pts {
            kern.generate_page_tables(manifest.security, &manifest.map_page_tables, riscv_enabled, vf2_enabled);
        }
        kern.set_attestation_hash();
        kern
    } else {
